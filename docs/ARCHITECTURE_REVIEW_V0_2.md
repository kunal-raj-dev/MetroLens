# ARCHITECTURAL REVIEW & TOPOLOGY UPGRADE (V0.2)
...
**Core Assessment:** Formal transition from over-extended V0.1 architecture to resilient, experiment-backed, edge-native V0.2 architecture.

---

## 1. Architectural Evolution: V0.1 vs. V0.2
...
================================================================================
  ...
  [Python Rule Functions] ───> [Automated Section 36 Penalty / Fine Issuer]
           │                                      │
           ▼                                      ▼
  [Form A PDF Notice] ───────> [Live National eMaap Webhook Sync] (UNVERIFIED API)

================================================================================
                          ARCHITECTURE V0.2 (CORRECTED)
================================================================================
  [Physical Package + Metric Anchor] ──> [Quality Guard: Glare & Blur Filter]
                                  ...
                                  • Rule 9 Table 1 Area & Font Matrix
...
- **What Was Wrong:** V0.1 asserted that fitting an ellipse to an Indian 10-Rupee coin contour uniquely resolves an 8-DOF planar homography matrix ($H$) to unwarp perspective tilt up to $35^\circ$ with sub-0.12mm accuracy.
- **Why It Is Problematic:** Mathematically, a circle projected under perspective forms an ellipse whose major and minor axes provide metric scale along the major axis and tilt inclination angle $\cos\theta = b/a$. However, it leaves the azimuthal orientation around the surface normal ambiguous without coplanar corner correspondences (such as package boundaries or rectangular markers). Treating this as a settled, production-ready algorithm before running an empirical spike risked catastrophic pipeline failure on Day 3.
- **V0.2 Correction:** Replaced the unverified assumption with a formal **Day 1 Calibration Feasibility Spike**. The pipeline decouples scale estimation ($S = \text{diameter} / d_{\text{major}}$) for near-normal captures from full homography, introduces standard rectangular ID card fallback ($85.60 \times 53.98\text{mm}$ providing 4 true corners), and enforces viewfinder plane-alignment guides.

### Defect 2: Statutory Misnomer & Illegal Enforcement Logic
- **What Was Wrong:** V0.1 generated a document titled "Form A Statutory Penalty Notice", cited Section 36 penalties up to ₹25,000, and framed the output as an automated fine.
- **Why It Is Problematic:** Under the Legal Metrology (Packaged Commodities) Rules, 2011, "Form A" is an application for manufacturer registration under Rule 27; it has nothing to do with inspection reports. Furthermore, under the **Jan Vishwas (Amendment of Provisions) Act, 2023**, Section 36(1) labeling violations have been decriminalized: the first offence mandates an **Improvement Notice** giving the brand time to rectify, with zero initial fine. Adjudication of repeated offences is restricted to statutory Adjudicating Officers under Section 48A.
- **V0.2 Correction:** Completely removed "Form A". The output is renamed **"MetroLens AI — Image-Based Compliance Assessment Report"**. The system acts strictly as an **assistive evidentiary screening tool under Section 15**, generating prima facie evidence and recommending an Improvement Notice under Section 36(1).
...
- **V0.2 Correction:** **Excised E-Commerce Scraper from MVP entirely** (deferred to post-hackathon). Refactored eMaap integration to an **"eMaap-Inspired Architecture / Mock REST Adapter"** with standardized JSON export, saving over 16 engineering hours.
...
- **Why It Is Problematic:** In field regulatory enforcement, low-contrast packaging, borderline font heights ($1.45\text{mm}$ vs $1.50\text{mm}$), or faded dot-matrix inkjet dates cannot be definitively classified as illegal without risking regulatory harassment.
- **V0.2 Correction:** Implemented a **5-State Compliance Model**:
...
## 3. Corrected Component Architecture (V0.2 Specifications)
...
| **Statutory Rule Engine**| `modules/rules/rule_engine.py` | Versioned Python State Machine | Deterministic evaluation of Rules 6(1)(a)-(h), 6(11) USP, 7, 8, 9 Table 1, and 26. | Graceful fallback: borderline values routed to `MANUAL_REVIEW_REQUIRED`. |
