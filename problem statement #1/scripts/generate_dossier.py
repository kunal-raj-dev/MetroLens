# -*- coding: utf-8 -*-
"""
Script to generate the comprehensive, decision-grade SIH26034 research dossier
and compile it into a publication-grade PDF using headless Microsoft Edge.
"""
...
md_path = r"c:\Users\kunal\Desktop\SIH\problem statement #1\SIH26034_Dossier.md"
pdf_path = r"c:\Users\kunal\Desktop\SIH\problem statement #1\SIH26034_Dossier.pdf"
html_path = r"c:\Users\kunal\Desktop\SIH\problem statement #1\SIH26034_Dossier.html"

print("Generating Markdown dossier...")

# We will construct the exhaustive dossier markdown content
dossier_content = """# DECISION-GRADE STRATEGIC RESEARCH DOSSIER: SIH26034
...
- **Computer Vision Reality:** 100% mathematically verifiable. Software performs an automated arithmetic audit:
  $$\text{Expected USP} = \frac{\text{Extracted MRP}}{\text{Extracted Net Quantity}}$$
...
- **Computer Vision Reality:** Highly verifiable. Deterministic regex checks for presence of a valid 10-digit Indian telephone/toll-free number and a syntactically valid email address (`\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b`).
...
- Focal length in pixels: 
  $$f_{px} = \frac{F \times W_{pixels}}{W_{sensor}} = \frac{5.4 \times 4000}{7.0} \approx 3085\text{ pixels}$$

When an inspector captures a label from a typical macro working distance $Z = 200\text{ mm}$ (20 cm):
$$\text{Spatial Resolution on Packaging Plane} = \frac{Z}{f_{px}} = \frac{200\text{ mm}}{3085\text{ px}} \approx 0.0648\text{ mm/pixel} \approx 65\text{ microns/pixel}$$

For the minimum statutory font heights prescribed under Rule 9 Table 1:
- **1.0 mm Font Height:** Resolves to $1.0 / 0.0648 \approx \mathbf{15.4\text{ pixels}}$
- **1.5 mm Font Height:** Resolves to $1.5 / 0.0648 \approx \mathbf{23.1\text{ pixels}}$
- **2.5 mm Font Height:** Resolves to $2.5 / 0.0648 \approx \mathbf{38.6\text{ pixels}}$
- **4.0 mm Font Height:** Resolves to $4.0 / 0.0648 \approx \mathbf{61.7\text{ pixels}}$
...
3. The image is warped via $H^{-1}$, creating an **orthorectified metric image** where perspective distortion is completely eliminated and every pixel has a constant millimeter scale factor:
   $$\text{Scale Factor } S = \frac{27.0\text{ mm}}{\text{Major Axis of Detected Coin in Pixels}}\quad (\text{mm/pixel})$$
...
- Along the horizontal circumferential axis ($X$), surface distance $R\Delta\phi$ projects to image width $w_{proj} = R(\sin\phi_2 - \sin\phi_1) \approx R\Delta\phi\cos\phi$. Thus, horizontal text is compressed as it approaches the cylinder edges.
...
print(f"Markdown dossier successfully written to: {md_path}")
print(f"Total size: {os.path.getsize(md_path)} bytes")

# Now compile to high-grade HTML and then PDF using Edge
...
html_body = markdown.markdown(dossier_content, extensions=['tables', 'fenced_code'])

full_html = f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>SIH26034 Strategic Research Dossier</title>
<style>
    @page {{
        size: A4;
...
print(f"Compiling PDF via Edge headless ({edge_path})...")
...
try:
    ...
    res = subprocess.run(cmd, check=True, timeout=60, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
